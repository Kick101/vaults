##### Domains in Scope
-   *.freshworks.com
-   *.freshdesk.com (Support Desk solutions for orgs)
-   *.freshservice.com (IT assets management, Project planning, )
-   *.freshsales.io (Sales management for orgs)
-   *.freshcaller.com (Customer support for orgs)
-   *.freshchat.com (Live chat solution for orgs to interact with their customers)
-   *.freshmarketer.com (Down?!)
-   *.freshconnect.io (Used to connect other orgs to freshworks workspace)
-   *.freshping.io (Website monitoring)

---
### freshmarketer.com
#### 404 
http://fmtracks.freshmarketer.com/
http://fwtracks.freshmarketer.com/

---
### freshping.io
#### 404
http://fmtracks.freshping.io/

#### support.freshping.io
__/en/support/discussions/topics/new__
Post a new topic
- Topic title
- Message
- Post topic in
	- Community Discussions
	- Ideas
- Attach files
- Catcha
- Save button

__/en/support/tickets__
- All Tickets
- Sorted by Date Created

__/en/support/profile/edit__
- Contact Territory
- Full name
- Title
- Email
- Work Phone
- Mobile phone
- Twitter
- Company
- Time zone
- Language
- Save button
- Change Password functionality

__/en/support/tickets/new__
- Submit a ticket
```sql
Liquid error: Mysql2::Error: [mysql_shard_35.master_proxy] Lost connection to MySQL server during query: SELECT `users`.* FROM `users` INNER JOIN `user_companies` ON `users`.`id` = `user_companies`.`user_id` WHERE `users`.`account_id` = 2 AND `user_companies`.`account_id` = 2 AND `user_companies`.`company_id` = 50000222690 AND (`users`.`deleted` = 0) ORDER BY name /*request_id:54acfa3a-61e3-44c1-946d-50b8615d81a7,replica:primary*/
```

