...
#### Security Groups
- Groups can have users machines and other groups.
- These are used to _grant permissions over resources_.

```powershell
Get-ADGroup -Identity "Schema Admins" -Properties *
```

__Default Security Groups__

![[Pasted image 20230225013725.png]]

#### Organizational Units (OUs)
_Container objects_ that allow you to classify users and machines. OUs are mainly used to define sets of users with similar _policing_ requirements. A user can only be a part of a _single OU at a time_.
...
- _Managed Service Accounts:_ Holds accounts used by services in your Windows domain.
...
- SYSVOL share points by default to the `C:\Windows\SYSVOL\sysvol\` directory on each of the DCs
...
Two Network authentication protocols in windows domains:
...
__NTLM Pass-Through Authentication__ Used to get access from a server
...
__NTLM Authentication__ Used to authenticate with the DC
...
Having a trust relationship _doesn't automatically grant access to all resources_ on other domains. Once a trust relationship is established, you have the chance to authorise users across different domains.
