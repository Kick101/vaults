...
This [link](http://www.kouti.com/tables/userattributes.htm) contains a large listing of User Attributes, and this [link](http://www.kouti.com/tables/baseattributes.htm) lists out all Base Attributes.
...
To interpret UAC attribute values, we can use this [script](https://academy.hackthebox.com/storage/resources/Convert-UserAccountControlValues.zip)
...
__Domain name, any child domains, a list of DC, DC roles__
```powershell
...
__Domain Accts__
```powershell
SharpView.exe Get-DomainUser * -AdminCount | select samaccountname,useraccountcontrol
```

__Names of all OUs__
```powershell
SharpView.exe Get-DomainOU | findstr /b "name"
```

__Domain users info and properties (Kerberosroasting)__
```powershell
...
__Group Policy Objects names__
```powershell
SharpView.exe Get-DomainGPO | findstr displayname
```

__Determine which GPOs map back to which hosts__
```powershell
...
__check if our current user has local admin rights on any remote hosts__
```powershell
Test-AdminAccess -ComputerName SQL01
```

__Enumerate open shares on a remote computer__
```powershell
...
__All domain trusts for our current domain__
```powershell
Get-DomainTrust
```

---
### Enumerating AD Users w/ SharpView/PowerVIew
...
__Export all users properties to a CSV__
```powershell
...
__Kerberos unconstrained delegation__
```powershell
...
__Enumerate users from other domain with group membership with any groups in our domain__
- If we can crack this user hash then we can authenticate into another domain

```powershell
Find-ForeignGroup
```
- Convert _MemberName_ SID to gather username
```powershell
Convert-SidToName S-1-5-21-888139820-103978830-333442103-1602
```

__Checking for SPNs accts in other domains that we can authenticate__
- Kerberoast across the forest trust
```powershell
Get-DomainUser -SPN -Domain otherdomain.local | select samaccountname,memberof,serviceprincipalname | fl
```
...
__Domain Groups__
- _Exchange Trusted Subsystem & Exchange Windows Permissions_ are considered [high-value targets](https://github.com/gdedrouas/Exchange-AD-Privesc)
```powershell
Get-DomainGroup -Properties Name
```

__Members of a Group__
```powershell
SharpView.exe Get-DomainGroupMember -Identity 'Help Desk'
```

__Protected Groups__
```powershell
SharpView.exe Get-DomainGroup -AdminCount
```

__Managed Groups__
- These groups have delegated non-administrators the right to add members to AD security groups and [distribution groups](https://docs.microsoft.com/en-us/exchange/recipients-in-exchange-online/manage-distribution-groups/manage-distribution-groups) and is set by modifying the _managedBy_ attribute.
```powershell
...
__Get user's ACL__
- _GenericWrite_ privileges -> (add or remove users).
```powershell-session
...
__Local Groups__
- Check local group membership. Is our current user local admin or part of local groups on any hosts
```powershell
Get-NetLocalGroup -ComputerName WS01 | select GroupName
```

__Enumerate members of local groups on any computer__
```powershell
